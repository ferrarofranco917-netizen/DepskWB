...
class BudgetWise {
    constructor() {
        ...
        this.defaultCategories = ['Alimentari', 'Trasporti', 'Altro']; // Limitato a 3 per free
...
updateFixedStatusHome() {
    ...
    updateSavingsMessages(percent, goal, currentSavings) {
        ...
        if (targetDateEl) {
            targetDateEl.textContent = monthsToGoal === Infinity ? 'Mai' : dateStr;
        }
        
        if (currentPlanEl) {
            currentPlanEl.innerHTML = monthsToGoal === Infinity 
                ? 'Con questi parametri non raggiungerai l\'obiettivo'
                : `Al ritmo attuale, raggiungerai l'obiettivo il <strong>${dateStr}</strong>`;
        }
    ...
    updateSavingsSuggestion(currentPercent, currentMonths, monthlyIncome, remaining) {
        ...
        if (bestPercent > currentPercent && bestMonths < currentMonths) {
            ...
            suggestionEl.innerHTML = `Aumenta al <strong>${bestPercent}%</strong> per arrivare <strong>${monthsSaved} mesi prima</strong>!`;
    ...
    applySavingsSuggestion(newPercent) {
        ...
        if (slider && percentageValue) {
            ...
            this.saveData();
            this.updateSavingsWidget();
            this.showToast(`💡 Suggerimento applicato: ${newPercent}% di risparmio`, 'success');
        }
    }
...
fileInput.addEventListener('change', async function(e) {
    ...
    this.updateLicenseStatus = () => {
        ...
        const planInfo = this.license.getPlanInfo();
    ...
    this.setupPremiumEventListeners = () => {
        ...
        if (activateLicenseBtn) {
            activateLicenseBtn.addEventListener('click', () => this.showLicenseModal());
        }
        
        const closePremiumBtn = document.getElementById('closePremiumBtn');
        if (closePremiumBtn) {
            closePremiumBtn.addEventListener('click', () => this.hidePremiumModal());
        }
        
        // License modal buttons
        const confirmLicenseBtn = document.getElementById('confirmLicenseBtn');
        if (confirmLicenseBtn) {
            confirmLicenseBtn.addEventListener('click', () => this.activateLicense());
        }
        
        const cancelLicenseBtn = document.getElementById('cancelLicenseBtn');
        if (cancelLicenseBtn) {
            cancelLicenseBtn.addEventListener('click', () => this.hideLicenseModal());
        }
    };
    ...
    this.showPremiumBannerIfNeeded = () => {
        ...
        if (banner && !this.license.hasFullPremiumAccess()) {
            banner.style.display = 'block';
        }
    };

    this.setupPremiumSystem = () => {
        // Aggiorna UI status licenza
        this.updateLicenseStatus();
        
        // Setup event listeners premium
        this.setupPremiumEventListeners();
    ...
    // ========== SISTEMA PREMIUM ==========
    const self = this; // Salva il riferimento corretto al this
    setTimeout(() => {
        self.setupPremiumSystem();
    }, 100); // Piccolo ritardo per garantire che tutto sia caricato
    ...
    this.startTrial = async () => {
        if (this.license.startTrial()) {
            this.showToast('🎁 Prova Premium attivata! 7 giorni gratuiti');
            this.updateLicenseStatus();
            this.hidePremiumModal();
            this.enablePremiumFeatures();
        } else {
            this.showToast('⚠️ Prova già utilizzata');
        }
    };

    this.activateLicense = async () => {
        const email = document.getElementById('licenseEmail').value;
        const key = document.getElementById('licenseKey').value;
        
        if (!email || !key) {
            this.showToast('⚠️ Compila tutti i campi');
            return;
        }
        
        if (await this.license.activateLicense(email, key)) {
            this.showToast('✅ Licenza Premium attivata!');
            this.updateLicenseStatus();
            this.hideLicenseModal();
            this.enablePremiumFeatures();
        } else {
            this.showToast('❌ Licenza non valida');
        }
    };

    this.enablePremiumFeatures = () => {
        // Abilita funzionalità premium nell'UI
        document.querySelectorAll('.feature-locked').forEach(el => {
            el.classList.remove('feature-locked');
        });
        
        // Nascondi banner premium
        const banner = document.getElementById('premiumBanner');
        if (banner) {
    ...
    this.checkFeatureLimit = (feature, currentCount = 0) => {
        if (!this.license) {
            console.warn('⚠️ License system non disponibile - feature check fallback');
            return true; // Permette uso in fallback
        }
        
        if (!this.license.canUseFeature(feature)) {
            this.showUpgradePrompt(feature);
            return false;
        }
        
        if (feature === 'transactions' && !this.license.canAddTransaction(currentCount)) {
            this.showToast(`⚠️ Hai raggiunto il limite di ${this.license.getCurrentLimits().maxTransactions} transazioni (versione Free)`);
    ...
    this.showUpgradePrompt = (feature) => {
        ...
        // Inserisci dopo l'elemento corrente o in cima al container
